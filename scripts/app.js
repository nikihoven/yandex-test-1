"use strict";
function renderWaterfall(rootNode, columnCount, elementGap, itemPadding = 15, itemBgColor = 'rgba(0, 0, 0, 0.6)') {
    // Получение нод рута
    const nodes = [...rootNode.children];
    // Добавление информации из ноды в массив с типом ParsedNodes
    const dataFromNodes = [];
    nodes.forEach(el => {
        dataFromNodes.push({ content: el.innerHTML, length: el.innerHTML.length });
    });
    ...
    dataFromNodes.forEach(t => {
        ...
        newNode.style.padding = itemPadding + 'px';
        newNode.style.backgroundColor = itemBgColor;
        newNode.innerHTML = t.content;
        columns[columnWithMinimalContent][0].appendChild(newNode);
...
document.body.prepend(renderWaterfall(rootNode, 3, 30));
